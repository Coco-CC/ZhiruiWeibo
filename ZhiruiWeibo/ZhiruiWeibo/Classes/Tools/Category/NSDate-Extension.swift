...
extension NSDate {
    class func createDateString(createAtStr : String) -> String {
        ...
        // 5.4.创建日历对象
        let calendar =  NSCalendar.current//NSCalendar.current
      ...
      let cmps = calendar.dateComponents([Calendar.Component.year,Calendar.Component.year], from: createDate, to: nowDate as Date) //components(.Year, fromDate: createDate, toDate: nowDate, options: [])
