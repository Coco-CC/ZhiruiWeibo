...
class PopoverAnimator: NSObject {
    ...
    public var callBack : ((_ presented : Bool) -> ())?
...
extension PopoverAnimator : UIViewControllerTransitioningDelegate {
    ...
    func presentationController(forPresented presented: UIViewController, presenting: UIViewController?, source: UIViewController) -> UIPresentationController? {
        ...
        let presentation = CCPresentationController(presentedViewController: presented, presenting: presenting)
        
        
        
        presentation.presentedFrame =  presentedFrame// CGRect(x: 100, y: 55, width: 180, height: 250)
...
extension PopoverAnimator : UIViewControllerAnimatedTransitioning {
    ...
    private func animationForPresentedView(transitionContext: UIViewControllerContextTransitioning) {
        ...
        presentedView.layer.anchorPoint = CGPoint(x: 0.5, y: 0.0)//CGPointMake(0.5, 0)
        UIView.animate(withDuration: transitionDuration(using: transitionContext), animations: { () -> Void in
            presentedView.transform = .identity //CGAffineTransformIdentity
        }) { (_) -> Void in
            // 必须告诉转场上下文你已经完成动画
